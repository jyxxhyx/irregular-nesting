...
def check_feasibility_distance(solution, problem: Problem, scale):
    ...
    for (index1, shape1), (index2, shape2) in combinations(enumerate(problem.shapes), 2):
        ...
        for point in polygon1:
            ...
            if temp_distance * scale < problem.material.spacing:
                ...
                logger.error('Shapes {} and {} are too close to each other'.format(shape1.shape_id, shape2.shape_id))
                logger.error('Point {} to shape {}\'s distance is {:.3f}'.format(point, shape2.shape_id, temp_distance))
    if feasibility_flag:
