...
def bottom_left_heuristic(problem: Problem, sequence, nfps, config) -> Dict[int, Position]:
    ...
    for hole in material.holes:
        nfp_hole = generate_nfp(shape.offset_polygon, hole.regular_polygon, config['clipper'])
    ...
    for outer_iter, idx in enumerate(sequence[1:]):
        ...
        for inner_iter, polygon in enumerate(positioned_polygons):
            ...
            nfp_polygon = _get_nfp(shape, positioned_shape, nfps, position, config['clipper'])
            if config['is_debug']:
...
def _get_nfp(next_shape: Shape, positioned_shape: Shape, nfps: dict, position: Position, config_clipper):
    """
